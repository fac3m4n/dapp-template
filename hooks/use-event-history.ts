...
import {
  ContractAbi,
  ContractName,
  UseDappEventHistoryConfig,
  UseDappEventHistoryData,
} from "@/utils";
 ...
 * @param config.watch - if set to true, the events will be updated every pollingInterval milliseconds set at dappConfig (default: false)
 * @param config.enabled - set this to false to disable the hook from running (default: true)
 */
export const useDappEventHistory = <
...
>({
  ...
  watch,
  enabled = true,
}: UseDappEventHistoryConfig<TContractName, TEventName, TBlockData, TTransactionData, TReceiptData>) => {
  ...
  const query = useInfiniteQuery({
    ...
    select: data => {
      ...
      const eventHistoryData = events?.map(addIndexedArgsToEvent) as UseDappEventHistoryData<
        TContractName,
        TEventName,
        TBlockData,
        TTransactionData,
